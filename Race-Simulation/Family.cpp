...
double Family::bonus_safety = 0.1;

Family::Family()	{}

double Family::get_bonus_safety() const
{
	return bonus_safety;
}

void Family::update_safety()
{
	set_safety(get_safety() * (1 + bonus_safety));
}
...
void Family::bonus()
{
	...
	{
		update_safety();
		cout << "! BONUS ! " << get_name() << ": " << safety_temp << " SAFETY ---> " << get_safety() << " %." << endl;
	}
}

void Family::race(Car& c, int length, int laps)
{
